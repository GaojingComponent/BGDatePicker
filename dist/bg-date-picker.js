...
/***/ function(module, exports, __webpack_require__) {
	...
	datePicker.directive('bgDatePicker', ['$compile', function ($compile) {
	    ...
	    return {
	        restrict: 'A',
	        require: ['^ngModel'],
	        link: function link($scope, iElm, iAttrs, ctrl) {
	            var ngModel = ctrl[0];
	            ...
	            iElm.on('click', function (e) {
	                ...
	                if (calendar == null) {
	                    ...
	                    var datePickerCalendar = $compile('<bg-date-picker-calendar on-date-click="' + iAttrs.onDateClick + '" ng-model="' + iAttrs.ngModel + '" input-id="' + id + '" id="' + randomId + '" style="left:' + left + 'px;top:' + top + 'px;" init-value="' + ngModel.$modelValue + '"></bg-date-picker-calendar>')($scope);
...
/***/ function(module, exports) {
	...
	bgDateService.service('DateService', [function () {
	    return {
	         ...
	         */
	        getMaxDayOfMonth: function getMaxDayOfMonth(year, month) {
	            switch (month) {
	                case 1:
	                case 3:
	                case 5:
	         ...
	         */
	        getFirstDayOfMonth: function getFirstDayOfMonth(year, month) {
	            var firstDay = 0;
	            ...
	            firstDay = Math.floor(century / 4) - 2 * century + y + Math.floor(y / 4) + Math.floor(13 * (month + 1) / 5);
	            firstDay = firstDay % 7;
	         ...
	         */
	        getLastMonth: function getLastMonth(year, month) {
	            if (month === 1) {
	                return {
	                    year: year - 1,
	                    month: 12
...
/***/ function(module, exports) {
	...
	bgDateCalendar.directive('bgDatePickerCalendar', ['$filter', '$parse', 'DateService', function ($filter, $parse, dateService) {
	     ...
	     */
	    var renderCalendar = function renderCalendar(year, month, date, table) {
	        var now = new Date();
